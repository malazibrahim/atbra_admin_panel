    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','profile_verification_mail_status_user')->first())
                    ...
                    <div class="card-body">
                        <div class="maintainance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                {{translate('Send_Mail_On_email_verification')}}
                                <span class="input-label-secondary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('Customers_will_receive_an_OTP_on_their_email_for_email_verification.')}}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                data-title-on="{{translate('Want_to_enable_Email_verification_otp_mail?')}}"
                                data-title-off="{{translate('Want_to_disable_Email_verification_otp_mail?')}}"
                                data-text-on="<p>{{translate('If_enabled,_customers_will_receive_a_Email_verification_otp_on_their_email.')}}</p>"
                                data-text-off="<p>{{translate('If_disabled,_customers_will_not_receive_any_OTP_for_email_verification.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['user','profile-verification',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','user')->where('email_type', 'profile_verification')->first())
                @php($template= $template ?? $data?->email_template ?? 4)
                <form action="{{ route('admin.business-settings.email-setup', ['user','profile-verification']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->with('translations')->where('type','user')->where('email_type', 'profile_verification')->first())
